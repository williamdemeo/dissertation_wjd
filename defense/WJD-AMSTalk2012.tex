...
\title[Overalgebras]{Expansions of Finite Algebras\\
  {\normalsize and their congruence lattices}}
...
\date[AMS Meeting]{ % (optional, should be abbreviation of conference name)
...
%\includeonlyframes{titlepage,problem,milestones,methods,knownresults,filterideal,MO,freese,OA,OAop2,OAop3,OAop4,OAcong,OAEx2,conclusion,MO,Conclusion}%  WORKS! PAP1,UAcalc,Extensions,Conclusion}
...
\frame[label=milestones]{
  ...
  \begin{columns}
    \begin{column}{0.8\textwidth}
      %% \begin{theorem}[{\small Quackenbush and Wolk, 1970}]
...
\frame[label=methods]{
  ...
  %% \begin{columns}
  %%   \begin{column}{0.8\textwidth}
  \begin{block}{Method 0}{Write the given lattice in terms of other representable lattices.}
...
\vskip2mm
    ...
    \item If $L$ is representable, so is 
      ...
      \item[$\cdot$] the dual of $L$\\[4pt]
      \item[$\cdot$] any interval sublattice of $L$\\[8pt]
      \end{itemize}
    \item If $L_1$ and $L_2$ are representable, so is\\[4pt]
      \begin{itemize} %[label=$\circ$]
      \item[$\cdot$]  the direct product of $L_1$ and $L_2$\\[4pt]
      \item[$\cdot$]  the ordinal sum of $L_1$ and $L_2$\\[4pt]
      \item[$\cdot$]  the parallel sum of $L_1$ and $L_2$
      \end{itemize}
    \end{itemize}
  \end{block}
...
\frame[label=methods]{
  \frametitle{How to find a concrete representation of a finite lattice}
  %% \begin{columns}
  %%   \begin{column}{0.8\textwidth}
  \begin{block}{Method 1 (closure)}{Find a ``closed'' representation of $L$ in $\Eq(X)$.}
    ...
    \item[] The map $\rho \lambda$ is a \emph{closure operator} on $\Sub[\Eq(X)]$.\\
      ...
      \vskip2mm
    \item[] If a lattice $L\leq \Eq(X)$ is \emph{closed}, i.e. $\rho\lambda(L) = L$, then
      \[L = \Con \<X, \lambda(L)\>\]
    \end{itemize}
  \end{block}
...
\frame[label=methods]{
  \frametitle{How to find a concrete representation of a finite lattice}
  \begin{block}{Method 2 (subgroup lattice interval)}{Find $L$ as an interval in a subgroup lattice of a finite group.}

    \vskip3mm

    If $H< G$ are finite groups, then the interval above $H$ in $Sub(G)$,
    \[
      [H,G] := \{K : H\leq K \leq G\},
      \]
  ...
  \begin{block}{Method 3 (filter+ideal)}{Find $L$ as the union of a filter and ideal in a representable lattice.}
        ...
        \item<2-> ...so the dual $L_{16}$ is also representable.
          \\[8pt]
            {\small $L_{16}$ can be embedded above diagonal of the direct power of
              a simple group,
              \[L_{16} \hookrightarrow [\Gamma,S^{48}] \cong \Eq(48)^{dual}.\]  
    ...
    \begin{column}{0.2\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=.3]
          ...
          \input{L3.tex}  \draw (1,-1) node {$L_{11}$};
    ...
    \begin{column}{0.2\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=.3]
          ...
          \input{L11.tex}  \draw (1,-1) node {$L_{11}$};
...
\begin{frame}[label=conclusion,shrink=5]{
    Concluding Remarks}
  \begin{columns}
    \begin{column}{0.2\textwidth}
      \begin{center}
...
\frame[label=MO,shrink=5]{
  ...
  \begin{center}
    \includegraphics[height=2.8in]{MOquestion}
  \end{center}
}  
